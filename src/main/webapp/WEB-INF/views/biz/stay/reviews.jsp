...
<script>
    ...
    if (pendingRequests.length > 0) {
      ...
      pendingRequests.forEach(function (dr) {
        ...
        html += buildReviewItemHtml({
          id: dr.reviewId, author: dr.author, dateLabel: '요청일 ' + dr.reqDate,
          rating: dr.rating, content: dr.content, reply: null
        }, { showActions: false, statusBadge: statusBadgeHtml('PENDING'), extraHtml: extra });
      });
    }
    if (!html) {
  ...
  function renderDeleteRequestHistory(box, statusCd, emptyMsg) {
    ...
    list.forEach(function (dr) {
      ...
      html += buildReviewItemHtml({
        id: dr.reviewId, author: dr.author, dateLabel: '요청일 ' + dr.reqDate,
        rating: dr.rating, content: dr.content, reply: null
      }, { showActions: false, statusBadge: statusBadgeHtml(statusCd), extraHtml: extra });
    });
    box.innerHTML = html;
  }
  ...
  function renderList() {
    ...
    box.innerHTML = '';
    if (currentTab === 'pending') { renderPendingTab(box); return; }
    if (currentTab === 'approved') { renderDeleteRequestHistory(box, 'APPROVED', '삭제 승인 내역이 없습니다.'); return; }
    if (currentTab === 'rejected') { renderDeleteRequestHistory(box, 'REJECTED', '삭제 반려 내역이 없습니다.'); }
  }
